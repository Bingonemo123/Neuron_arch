...
class main ():
    ...
    def mainloop (self, Informator):
        while self.origin_date <= datetime.datetime.today() and self.k != 103:
            ...
            if len(self.Inf) > self.diapason:
                ...
                for ant in self.Fungus.nationaleconomy:
                    try:
                        loss = 0 if (int(ant[1][1])%3)-1 == flag else 1
                ...
                speech = [self.origin_date.date(),
                            flag ,
                            100-sum([ant[1] for ant in priority]),
                            self.Fungus.population[0][0].axonsum(),
                            self.Fungus.nationaleconomy[0][1]]
                logger.info('  '.join([str(sp) for sp in speech]))

                self.Fungus.population[0] = self.Fungus.axoncomplexitymaplocal([x[0] for x in priority if x[1] == 0]) + \
                                            self.Fungus.axoncomplexitymaplocal([x[0] for x in priority if x[1] == 1])
...
while ground.origin_date <= datetime.datetime.today() and ground.k != 103:
    try:
